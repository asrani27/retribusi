        ...
        <div class="row">
          <div class="col-12">
            <div class="card card-outline card-primary">
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{route('editlos',['id' => $data->id])}}">
                    @csrf
                  <div class="card-body">
                    ...
                    <div class="form-group row">
                      <label for="inputEmail3" class="col-sm-2 col-form-label">Nomor los</label>
                      <div class="col-sm-10">
                        <input type="text" class="form-control" name="nomor" value="{{$data->nomor}}">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="inputEmail3" class="col-sm-2 col-form-label">nama los</label>
                      <div class="col-sm-10">
                      ...
                      <div class="col-sm-10">
                        <select name="blok_id" class="form-control" required>
                          ...
                          @foreach ($blok as $item)
                              <option value="{{$item->id}}" {{$data->blok_id == $item->id ? 'selected':''}}>{{$item->nomor}}, luas : {{$item->luas}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                  ...
                  <div class="card-footer">
                    <button type="submit" class="btn btn-info"><i class="icon fas fa-check"></i>
                        Update</button>
                    <a href="/los" class="btn btn-default float-right">Kembali</a>
                  </div>
                  <!-- /.card-footer -->
                </form>
              </div>
