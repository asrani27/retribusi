        ...
        <div class="row">
          <div class="col-12">
            <div class="card card-outline card-primary">
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{route('editperalihan',['id' => $data->id])}}">
                    @csrf
                  <div class="card-body">
                      ...
                      <div class="col-sm-10">
                        <select name="pedagang_lama" class="form-control" required>
                          ...
                          @foreach ($pedagang_lama as $item)
                              <option value="{{$item->id}}" {{$data->pedagang_lama == $item->id ? 'selected':''}}>{{$item->nama}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                      ...
                      <div class="col-sm-10">
                        <select name="pedagang_baru" class="form-control" required>
                          ...
                          @foreach ($pedagang_baru as $item)
                              <option value="{{$item->id}}" {{$data->pedagang_baru == $item->id ? 'selected':''}}>{{$item->nama}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
