        ...
        <div class="row">
          <div class="col-12">
            <div class="card card-outline card-primary">
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{route('hasil')}}">
                    @csrf
                  <div class="card-body">
                      ...
                      <div class="col-sm-10">
                        <select name="jadwal_id" class="form-control" required>
                          ...
                          @foreach ($jadwal as $item)
                              <option value="{{$item->id}}">{{$item->infrastruktur->nama}}, {{$item->status}}, Tgl : {{\Carbon\Carbon::parse($item->tanggal)->format('d M Y')}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                    <div class="form-group row">
                      ...
                      <div class="col-sm-10">
                        <textarea rows="4" name="hasil_pemeliharaan" class="form-control"></textarea>
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="inputEmail3" class="col-sm-2 col-form-label">Hasil Penggantian</label>
                      <div class="col-sm-10">
                        <textarea rows="4" name="hasil_penggantian" class="form-control"></textarea>
                      </div>
                    </div>
                    <div class="form-group row">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-sm-10">
                        <input type="text" class="form-control" name="biaya" onkeypress="return hanyaAngka(event)"/>
                      </div>
                    </div>
                    <div class="form-group row">
